...
const handler: Handler<APIGatewayProxyEvent, APIGatewayProxyResult> = async (
  ...
  let response = await fetch(
    `https://raw.githubusercontent.com/apollographql/rover/${plugin_version}/installers/binstall/scripts/windows/install_${plugin_name.replace("-", "_")}.ps1`,
  );
  ...
  if (response.status === 404){
    return {
      statusCode: 400,
      body: `Couldn't find release for ${plugin_name}@${plugin_version} on GitHub Releases. This could be a problem with GitHub being offline or missing this version`,
    }
  }
