...
export async function getPluginFromEvent(
  ...
  if (!supportedPlugins.includes(plugin_name)) {
    throw new Error(
      `Plugin ${plugin_name} does not exist. Must be one of: ${supportedPlugins}`
    );
  }
  ...
  if (!platform) {
    throw new Error(
      `No platform provided. Must include one of: ${supportedPlatforms}`
    );
  }

  if (!version) {
    throw new Error(
      "No version provided. Versions must be `latest` or in semver format `v0.0.0`"
    );
  }
  ...
  if (!supportedPlatforms.includes(platform.toLowerCase())) {
    throw new Error(
      `Invalid platform: ${platform}. Supported platforms: ${supportedPlatforms}`
    );
  }
  ...
  if (version.toLowerCase() == "latest") {
    ...
    throw new Error(
      `Invalid version: (${version}). Versions must start with a 'v' or be 'latest'.`
    );
  }
}
